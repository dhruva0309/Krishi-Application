...
with st.sidebar:
    options = ["Home", "Rainfall Prediction", "Crop Yield Prediction", "Crop Recommendation", "AQI Prediction", "Meet the Creator"]
    selected = option_menu("Krishi Mitra",
                           options,
                           menu_icon=":seedling:",
...
# Meet Creators
    ...
    creators = [
        {
            "name": "K Dhruva Somayaji",
            "linkedin": " https://www.linkedin.com/in/dhruva-somayaji-405b78269/",
            "github": "https://github.com/dhruva0309",
            "image": "images/Dhruva_Photo.jpg"
        }
    ]
    ...
    for i, creator in enumerate(creators):
        with cols[0]:  # Column for image
            st.image(creator["image"], width=150, caption=None, use_column_width=True, output_format='auto')

        with cols[1]:  # Column for details
            st.markdown(f"<div class='profile-column' style='padding-left: 20px;'><p class='profile-name'>{creator['name']}</p><a href='{creator['linkedin']}' target='_blank'><img src='https://upload.wikimedia.org/wikipedia/commons/8/81/LinkedIn_icon.svg' class='icon'></a> <a href='{creator['github']}' target='_blank'><img src='https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png' class='icon'></a></div>", unsafe_allow_html=True)
...
# Home
    ...
    st.write("##### Welcome to Krishi - An Intelligent Helping Hand for Farmers! Explore our tools in the sidebar to make informed agricultural decisions.")
    st.image(img, width=750)
    
    st.write("")  # Leave some space after the image
    st.write("### Overview")  # Section title for introduction
    st.write("This application is designed to empower farmers with advanced predictive tools. Predict rainfall accurately with our Ridge Regression model. Get personalized crop recommendations using our Random Forest algorithm tailored to your local conditions. Estimate crop yields confidently with a combination of CatBoost, XGBoost, and Decision Tree models. Assess Air Quality Index (AQI) with precision using our XGBoost model, fine-tuned with Grid Search CV. Harness these features to optimize farming strategies and increase productivity.")
